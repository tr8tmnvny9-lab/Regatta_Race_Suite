...
class DynamicTacticalRenderer: MKOverlayRenderer {
    ...
    private func drawMarkZones(in context: CGContext, zoomScale: MKZoomScale) {
        ...
        for buoy in raceState.course.marks {
            ...
            // Calculate radius in map points
            // 1 map point is roughly 1 meter at the equator, but we should be more precise
            let pointsPerMeter = MKMapPointsPerMeterAtLatitude(center.latitude)
            let radiusPoints = radiusMeters * pointsPerMeter
            
            let rect = rect(for: MKMapRect(
                origin: MKMapPoint(x: mp.x - radiusPoints, y: mp.y - radiusPoints),
                size: MKMapSize(width: radiusPoints * 2, height: radiusPoints * 2)
            ))
    ...
    private func drawHeightToMarkLines(in context: CGContext, zoomScale: MKZoomScale) {
        ...
        guard let originMark = upperMark else { return }
        let originCoord = getCoord(originMark)
        let originMP = MKMapPoint(originCoord)
    ...
    private func intersectPoints(p1: MKMapPoint, p2: MKMapPoint, p3: MKMapPoint, p4: MKMapPoint) -> MKMapPoint? {
        ...
        // We only check t because p1-p2 is our infinite line, p3-p4 is the layline segment
        if u >= 0 && u <= 1 {
            return MKMapPoint(x: x1 + t * (x2 - x1), y: y1 + t * (y2 - y1))
        }
        return nil
    }
